 ...
 * component for the staggered entrance — visitors who prefer reduced motion
 * get a plain, non-animated render instead of the `motion` tree, matching
 * the pattern in `components/public/reveal.tsx`.
 */
export function Hero({ profile }: { profile: Profile | null }) {
  ...
  const content = reduceMotion ? (
    <div>
      ...
      <h1 className="mt-4 text-balance font-display text-5xl font-black leading-[0.95] tracking-tight text-foreground sm:text-6xl md:text-7xl">
        {name}
      </h1>
